...
const OmniKit = {
        ...
        modal.innerHTML = `<div class="relative w-full h-full max-w-md md:h-auto">
                                <div class="relative bg-white rounded-lg shadow dark-disable:bg-gray-700">
                                    ...
                                    <div class="p-6 text-center">
                                        ...
                                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark-disable:text-gray-400">${data.title ?? ''}</h3>
  ...
  cache: {
    ...
    settings: function () {
      ...
      let body =  ``;
      OmniKit.modal.create({title: `Instellingen`,body: body,modalUuid: uid, });
    ...
    cachePages: function () {
      ...
      let offset = 0;
      const loopCache = async (offset) => {
        await OmniKit.getData(`${OmniKit.pluginUrl}/ajax/cache/options.php?cachePages=true&offset=${offset}`)
        .then(data => {
          ...
          for (let key = 0; key < data.pages.length; key++) {
            ...
            item.innerHTML = `<tr class="bg-white border-b dark-disable:bg-gray-900 dark-disable:border-gray-700">
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark-disable:text-white">
                                    <strong class="black">${(Number(rows) + 1)}</strong>
                                </th>
                                <td class="px-6 py-4">
                                  ${page.name}
                                </td>
                                ...
                                <td class="px-6 py-4">
                                  <a href="${page.url}" class="font-medium text-blue-600 dark-disable:text-blue-500 hover:underline function-added" target="_blank">${page.url} </a>
                                </td>
          ...
          if(data.last == false){
            offset = 5 + offset;
            if(data.newOffset == true){ offset = 0;}
            loopCache(offset);
            return;
          }
          ...
          OmniKit.cache.fillTable(false,false,true);
        }).catch(error => console.log(error));
      };
      loopCache(offset);
    },
          ...
          for (let key in data){
            ...
            const item = document.createElement('tr');
                  item.innerHTML = `<tr class="bg-white border-b dark-disable:bg-gray-900 dark-disable:border-gray-700">
                                ...
                                <td class="px-6 py-4">
                                    <a href="${cachedPage.page}" target="_blank" class="font-medium text-blue-600 dark-disable:text-blue-500 hover:underline">Bekijken</a>
                                </td>
                            </tr>`;
            table.append(item);
          }
    ...
    deleteCache: function (confirm = false) {
      if(confirm){
        let formData = new FormData();
        let cashBtn = document.querySelectorAll('[data-omnikit-cache="deleteBtn"]')[0];
        let oldText = cashBtn.innerHTML;
        cashBtn.innerHTML = `<svg aria-hidden="true" role="status" class="inline w-4 h-4 mr-3 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="#E5E7EB"/><path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentColor"/></svg>Laden...`;
        OmniKit.submit({formData: formData,url: `${OmniKit.pluginUrl}/ajax/cache/options.php?delete=true`}).then(data => {
          if(data.error){
            OmniKit.toast.create({type: 'danger', title: (data.title ?? 'Er ging iets mis'),content: (data.content ?? ''), autoClose: true });
          }
          ...
          setTimeout(() => {
            cashBtn.innerHTML = oldText;
          }, 650);
        });
        return;
      }
      OmniKit.modal.create({type: 'confirm',title: "Weet u zeker dat u de cache wilt verwijderen",btnConfirm: "Cache wissen",confirmAction: `cache.deleteCache(true)`});      
